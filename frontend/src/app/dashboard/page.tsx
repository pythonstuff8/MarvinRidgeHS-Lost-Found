...
export default function Dashboard() {
    ...
    // Update item status (e.g., approve a pending item)
    ...
    // Update claim status and conditionally resolved unrelated item
    const handleClaimStatus = async (claimId: string, itemId: string, newStatus: string) => {
        try {
            ...
            if (newStatus === "APPROVED") {
                // If approved, mark the item as resolved/claimed
                            ...
                            {claims.length > 0 ? claims.map((claim) => (
                                <div key={claim.id} className="bg-white border border-gray-200 rounded-2xl p-4 md:p-6 shadow-sm">
                                    ...
                                    <div className="bg-gray-50 p-4 rounded-xl mb-4 border border-gray-100">
                                        <p className="text-xs text-gray-500 mb-1 font-bold uppercase">Proof of Ownership:</p>
                                        <p className="text-sm text-gray-800 break-words">"{claim.proof}"</p>
                                    </div>
